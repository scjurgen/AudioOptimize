...
{
  public:
    TestBiquadFilterSet(const DSP::BiquadFilterType ft_, const double gain_)
        : type(ft_)
        , gain(gain_)
    {
    }
...
static std::array<TestBiquadFilterSet, 9> filterTestSet{{{DSP::BiquadFilterType::LowPass, 0.0},
                                                         {DSP::BiquadFilterType::HighPass, 0.0},
                                                         {DSP::BiquadFilterType::BandPass, 0.0},
                                                         {DSP::BiquadFilterType::Peak, 6.0},
                                                         {DSP::BiquadFilterType::Peak, -6.0},
                                                         {DSP::BiquadFilterType::LoShelf, 6.0},
                                                         {DSP::BiquadFilterType::LoShelf, -6.0},
                                                         {DSP::BiquadFilterType::HiShelf, 6.0},
                                                         {DSP::BiquadFilterType::HiShelf, -6.0}}};
...
{
  public:
    void setType(DSP::BiquadFilterType type)
    {
        m_currentFilterType = type;
    }

    void computeCoefficients(DSP::BiquadFilterType type, const float sampleRate, const float frequency, const float Q,
    ...
    DSP::BiquadFilterType m_currentFilterType;
    DSP::Biquad<DSP::BiquadFilterType::LowPass> lp;
    DSP::Biquad<DSP::BiquadFilterType::HighPass> hp;
    DSP::Biquad<DSP::BiquadFilterType::BandPass> bp;
    DSP::Biquad<DSP::BiquadFilterType::Notch> notch;
    DSP::Biquad<DSP::BiquadFilterType::Peak> peak;
    DSP::Biquad<DSP::BiquadFilterType::LoShelf> ls;
    DSP::Biquad<DSP::BiquadFilterType::HiShelf> hs;
};
...
class WrapperOfSpecializedBiquadsStereo
{
  public:
    ...
    DSP::BiquadFilterType m_currentFilterType;
    DSP::BiquadStereo<DSP::BiquadFilterType::LowPass> lp;
    DSP::BiquadStereo<DSP::BiquadFilterType::HighPass> hp;
    DSP::BiquadStereo<DSP::BiquadFilterType::BandPass> bp;
    DSP::BiquadStereo<DSP::BiquadFilterType::Notch> notch;
    DSP::BiquadStereo<DSP::BiquadFilterType::Peak> peak;
    DSP::BiquadStereo<DSP::BiquadFilterType::LoShelf> ls;
    DSP::BiquadStereo<DSP::BiquadFilterType::HiShelf> hs;
};
...
class DspBiquadFilterFixtureTest : public ::testing::Test
{
  public:
    static constexpr size_t sampleRate{48000};
    void analyze(size_t order)
    {
        ...
        for (float hz = 125.f / 4.f; hz < 20000; hz *= 2)
        {
            ...
            sut.processBlock(wave.data(), out.data(), wave.size());
            {
                ...
                if (expectedDb < -60)
                {
                    EXPECT_LT(db, -60) << " failure @" << hz << " order:" << order;
                }
                else
                {
                    EXPECT_NEAR(db, expectedDb, 2.0f) << " failure @" << hz << " order:" << order;
                }
            }
        }
    }
...
TEST_F(DspBiquadFilterFixtureTest, chebyshevFilterType1LowpassMono)
{
    for (size_t order = 1; order < 10; ++order)
    {
        sut.computeType1(order, 1000, 2, true);
        analyze(order);
    }
}
TEST_F(DspBiquadFilterFixtureTest, chebyshevFilterType1HighpassMono)
{
    for (size_t order = 1; order < 10; ++order)
    {
        sut.computeType1(order, 1000, 2, false);
        analyze(order);
    }
}
...
TEST(DspBiquadFilterTest, chebyshevFilterType1LowpassMono)
{
    ...
    for (size_t order = 1; order < 10; ++order)
    {
        ...
        for (float hz = 125.f / 4.f; hz < 10000; hz *= 2)
        {
            renderWithSineWave(wave, sampleRate, hz);
            sut.processBlock(wave.data(), out.data(), wave.size());
            {
...
TEST(DspBiquadFilterTest, chebyshevFilterType1HighpassMono)
{
    for (size_t order = 1; order < 10; ++order)
    {
        ...
        DSP::ChebyshevBiquad sut;
        sut.setSampleRate(sampleRate);
        sut.computeType1(order, 1000.f, 2, false);
        std::vector<float> wave(48000, 0);
        std::vector<float> out(48000, 0);
        renderWithSineWave(wave, sampleRate, 2000.f);
        sut.processBlock(wave.data(), out.data(), wave.size());
        ...
        for (float hz = 100; hz < 10000; hz *= 1.7)
        {
            renderWithSineWave(wave, sampleRate, hz);
            sut.processBlock(wave.data(), out.data(), wave.size());
            {
    ...
    for (size_t order = 8; order < 10; ++order)
    {
        ...
        for (float hz = 10; hz < 20000; hz *= 2)
        {
            ...
            sut.processBlock(wave.data(), out.data(), wave.size());
            {
                ...
                if (expectedDb < -60)
                {
                    EXPECT_LT(db, -60); // allow 6dB noise floor
                }
                else
                {
                    EXPECT_NEAR(db, expectedDb, 2.0f) << "left channel failure @" << hz << " order: " << order;
