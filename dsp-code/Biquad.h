...
class BiquadCoefficients
{
  public:
    ...
    {
        const auto Fc = frequency / sampleRate;
        ...
        switch (type)
        {
            ...
            case BiquadFilterType::LoShelf:
            {
                ...
                const auto w0 = 2 * static_cast<float>(M_PI) * frequency / sampleRate;
...
class BiquadOriginal : public BiquadCoefficients
{
  public:
    ...
    {
        std::transform(in, in + numSamples, outBuffer, [this](const float in) { return step(in); });
    }

    void processBlock(float* inOut, size_t numSamples)
    {
        std::transform(inOut, inOut + numSamples, inOut, [this](const float in) { return step(in); });
    }
...
class Biquad : public BiquadCoefficients
{
  public:
    ...
    void processBlock(float* inOut, size_t numSamples)
    {
        processBlock(inOut, inOut, numSamples);
    }
    ...
    void processBlock(const float* in, float* outBuffer, size_t numSamples)
    {
        ...
        {
            case BiquadFilterType::LowPass:
                std::transform(in, in + numSamples, outBuffer,
                               [this](const float in) { return singleStepLowPass(in); });
                break;
            case BiquadFilterType::HighPass:
                std::transform(in, in + numSamples, outBuffer,
                               [this](const float in) { return singleStepHighPass(in); });
                break;
            case BiquadFilterType::BandPass:
                std::transform(in, in + numSamples, outBuffer,
                               [this](const float in) { return singleStepBandPass(in); });
                break;
            case BiquadFilterType::Notch:
                std::transform(in, in + numSamples, outBuffer, [this](const float in) { return singleStepNotch(in); });
                break;
            case BiquadFilterType::Peak:
                std::transform(in, in + numSamples, outBuffer, [this](const float in) { return singleStepPeak(in); });
                break;
            default: // low and high shelf and free coefficients
                std::transform(in, in + numSamples, outBuffer,
                               [this](const float in) { return singleStepGeneric(in); });
                break;
        }
    }
...
class BiquadStereo : public BiquadCoefficients
{
  ...
  private:
    ...
    std::array<std::array<float, 2>, 2> m_z{{{0, 0}, {0, 0}}};
};
...
class ChebyshevBiquad
{
  public:
    ...
    auto computeFactors(const size_t order, const float fc, const float ripple)
    {
        ...
        m_ripple = ripple;
        m_fc = fc;
        const auto fNorm = fc / m_sampleRate * 2;
        const auto eps = std::sqrt(std::pow(10.0f, std::max(0.001f, ripple) / 10.0f) - 1);

        return InitialFactors{std::tan(static_cast<float>(M_PI) * fNorm / 2),
                              std::cos(fNorm * static_cast<float>(M_PI)),
    ...
    {
        auto fDenom = fDenomR * fDenomR + fDenomI * fDenomI;
    ...
    void computeType1(const size_t order, const float fc, const float ripple, const bool isLowPass)
    {
        ...
        // biquads are 2 poles, create pairs of values
        for (auto iPair = 0u; iPair < order / 2; iPair++)
        {
            ...
            {
                fZZero = {-1, 0};
                fDCPoleDistance = std::norm(std::complex<float>{1, 0} - fZPole) / 4;
            }
            else
            {
                float fPoleX, fPoleY;
                ComplexDiv(beta - fZPole.real(), -fZPole.imag(), 1 - beta * fZPole.real(), -beta * fZPole.imag(),
                           fPoleX, fPoleY);
        ...
        // discriminate odd and even order
        if ((order & 1) == 0)
        {
            m_isOdd = false;
            float fTemp = dbToGain(-std::max(0.00001f, ripple)); // at DC the response is down R dB (for even-order)
        ...
        else
        {
            ...
            // compute 1st
            std::complex<float> fZZero;
            std::complex<float> fSPole{-fC * sinh(a), 0};

            auto fZPole = BilinearTransform(fSPole);
            // BilinearTransform(-fC * sinh(a), 0, fZPoleX, fZPoleY);
            auto fDCPoleDistance = 1.0f;
            if (isLowPass)
            {
                fZZero = {-1, 0};
                fDCPoleDistance = sqrt(std::norm(std::complex<float>{1, 0} - fZPole)) / 2;
            }
            else
            {
                fZPole = std::complex<float>{beta - fZPole.real(), -fZPole.imag()} /
            ...
            // this could be optimized in the single step because fZZeroReal = 1 or -1 -> b0 = +-b1
    ...
    void computeType2(const size_t order, const float fc, const float ripple, const bool isLowPass)
    {
        ...
        if (order & 1)
        {
            ...
            auto fSPole = std::complex<float>(fC, 0) /
                          std::complex<float>(-sinh(a) * sin((2 * iPair + 1) * static_cast<float>(M_PI) / (2 * order)),
                                              cosh(a) * cos((2 * iPair + 1) * M_PI / (2 * order)));
            auto fZPole = BilinearTransform(fSPole);
            float fZZeroReal;
            float fDCPoleDistance;
            if (isLowPass)
    ...
    // 3 x mul
    float stepChebyType1(const float in, const float b0, const float b1, const float b2, const float a1, const float a2,
                         float& z0, float& z1)
    {
        auto out = in * b0 + z0;
        z0 = in * b1 + z1 - a1 * out;
    ...
    void process1stOrder(size_t index, const float* in, float* out, const size_t numSamples)
    {
        for (size_t i = 0; i < numSamples; ++i)
        {
    ...
    void process1stOrderStereo(size_t index, const float* left, const float* right, float* outLeft, float* outRight,
    ...
    void processElement(size_t index, const float* in, float* out, const size_t numSamples)
    {
        if (m_isType1)
        {
            for (size_t i = 0; i < numSamples; ++i)
            {
                out[i] = stepChebyType1(in[i], coefficients[index].b0, coefficients[index].b1, coefficients[index].b2,
    ...
    void processElementStereo(size_t index, const float* left, const float* right, float* outLeft, float* outRight,
                              const size_t numSamples)
    {
        if (m_isType1)
        {
            ...
            {
                outLeft[i] =
                    stepChebyType1(left[i], coefficients[index].b0, coefficients[index].b1, coefficients[index].b2,
                                   coefficients[index].a1, coefficients[index].a2, z[index][0], z[index][1]);
                outRight[i] =
                    stepChebyType1(right[i], coefficients[index].b0, coefficients[index].b1, coefficients[index].b2,
    ...
    auto getMagnitudeInDb(float cf) const
    {
        float sum = 0;
        for (size_t i = 0; i < m_elements; ++i)
        {
